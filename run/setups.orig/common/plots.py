...
class PyFoamPlots(object):
    ...
    def plot_sets_contour(self, FO_name, file_name, data):
        ...
        for i,name in enumerate(data):
            ...
            fig, ax = plt.subplots()
            CS = ax.contourf(xv,yv,zv.transpose(),levels=25)
            CS = ax.contour(xv,yv,zv.transpose(),levels=25, colors='k')
...
if __name__ == '__main__':
    pfp = PyFoamPlots()
    pfp.plot_sets_contour(FO_name='sample', file_name='centre_Tf_Ts.xy', data=['y-position (m)', 'Fluid Temperature (K)', 'Solid Temperature (K)'])